...
export class PerformHealthCheckService {
   ...
   * @param probe - 基礎設施探測器 (Port)
   */
  constructor(
    private repository: IHealthCheckRepository,
    probe: IInfrastructureProbe
  ) {
    this.domainService = new HealthCheckService(probe)
  }
  ...
  async execute(): Promise<HealthCheckDTO> {
    ...
    const checks = await this.domainService.performSystemCheck()

    // 2. 創建聚合根，產生事件 (Domain Layer)
    const healthCheck = HealthCheck.perform(`health-${Date.now()}`, checks)
